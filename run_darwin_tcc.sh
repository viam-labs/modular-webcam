#!/bin/bash
# run_darwin.sh: macOS-specific entrypoint for camera TCC access.
#
# On macOS, viam-server runs as root, but TCC (Transparency, Consent, and
# Control) denies camera access to root processes. To get AVFoundation camera
# access, this script registers the module binary as a LaunchAgent in the
# console user's GUI session (gui/<uid>) — the only launchd domain where TCC
# consent applies. It then acts as a supervisor, blocking until the agent exits
# or receives SIGTERM.
